...
from src.matops.mat import MatDevice, MatOps, MatLib, NumpyMatOps, TorchMatOps, CupyMatOps
...
def project_camera_points_opencv_model(
    ...
    *,
    ...
    p = points_camera
    ops = OPS[(MatLib.which(p),MatDevice.which(p))]
...
def torch_example(device=MatDevice.CPU):
    device = torch.device(device)
    dtype = torch.float64

    make = lambda x: torch.tensor(x, dtype=dtype)
