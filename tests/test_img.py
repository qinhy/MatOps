...
def project_camera_points_opencv_model(points_camera, K, distortion=None, *, dtype, ops:MatOps):
...
for (lib, device), op in OPS.items():
    ...
    print('=' * 80)
    try:
        calib = StereoRgbCalibration.from_dict(DEFAULT_CALIBRATION, ops=op)
        ...
        img_np = np.arange(h * w, dtype=np.uint8).reshape(h, w)
        img = op.from_numpy(img_np)
        left_rect, right_rect, rect2 = rectifier.rectify(img, img)
